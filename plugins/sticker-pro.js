...
let handler = async (m, { conn, text, usedPrefix, command, args }) => {
    ...
    if (command === 'wm' || command === 'take' || command === 'robar') {
        ...
        try {
            let stiker = await addExif(img, packname || '***Lu Bot Prem***', author || '')
            await conn.sendFile(m.chat, stiker, 'lubotprem.webp', '', m)
            await m.react('✅')
        } catch (e) {
            console.error(e)
            await m.react('❌')
    ...
    if (command === 's' || command === 'sticker' || command === 'stiker') {
        ...
        let stiker = await sticker(img, false, '***Lu Bot Prem***', '')
        await conn.sendFile(m.chat, stiker, 'lubotprem.webp', '', m)
        await m.react('✅')
    }
    ...
    if (command === 'qc' || command === 'quotly') {
        ...
        try {
            ...
            const stiker = await sticker(buffer, false, '***Lu Bot Prem***', '')

            if (stiker) {
                await conn.sendFile(m.chat, stiker, 'lubotpremqc.webp', '', m)
                await m.react('✅')
            } else {
                await m.react('❌')
            }
    ...
    function error(msg) {
        let texto = `🐱 *𝗟𝗨 𝗕𝗢𝗧 𝗣𝗥𝗘𝗠 - 𝗦𝗧𝗜𝗖𝗞𝗘𝗥𝗦* 🐱
